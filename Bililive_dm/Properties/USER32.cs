...
internal partial class WINAPI
{
    internal static class USER32
    {
        ...
        private const string MODULENAME = nameof(USER32);
        ...
        [Obsolete] public const int GWL_EXSTYLE = -20;

        [Obsolete] public const int GWL_HINSTANCE = GWLP_HINSTANCE;

        private const int GWLP_HINSTANCE = -6;

        [Obsolete] public const int GWL_ID = GWLP_ID;

        private const int GWLP_ID = -12;

        [Obsolete] public const int GWL_STYLE = -16;

        [Obsolete] public const int GWL_USERDATA = GWLP_USERDATA;

        private const int GWLP_USERDATA = -21;

        [Obsolete] public const int GWL_WNDPROC = GWLP_WNDPROC;

        private const int GWLP_WNDPROC = -4;

        [Obsolete] public const int DWL_MSGRESULT = DWLP_MSGRESULT;

        private const int DWLP_MSGRESULT = 0;
...
#pragma warning disable CS0612 // 类型或成员已过时
        public static int GetWindowStyles(IntPtr hWnd, WindowStylesKind kind)
        {
            return GetWindowLong(hWnd, (int)kind);
        }
#pragma warning restore CS0612 // 类型或成员已过时
        public static ExtendedWindowStyles GetExtendedWindowStyles(IntPtr hWnd)
        {
            return (ExtendedWindowStyles)GetWindowStyles(hWnd, WindowStylesKind.ExStyles);
        }
...
#pragma warning disable CS0612 // 类型或成员已过时
        public static int SetWindowStyles(IntPtr hWnd, WindowStylesKind kind, int styles)
        {
            return SetWindowLong(hWnd, (int)kind, styles);
        }
#pragma warning restore CS0612 // 类型或成员已过时
        public static ExtendedWindowStyles SetExtendedWindowStyles(IntPtr hWnd, ExtendedWindowStyles styles)
        {
            return (ExtendedWindowStyles)SetWindowStyles(hWnd, WindowStylesKind.ExStyles, (int)styles);
        }
